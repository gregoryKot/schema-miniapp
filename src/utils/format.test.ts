...
describe('fmtDateLong', () => {
  ...
  it('обрабатывает май (одинаков в обоих списках)', () => {
...
describe('todayStr', () => {
  ...
  it('возвращает YYYY-MM-DD в локальной таймзоне', () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date(2026, 5, 8, 12, 0, 0)); // 8 июня 2026, локально
    expect(todayStr()).toBe('2026-06-08');
  });

  it('добавляет ведущие нули к месяцу и дню', () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date(2026, 0, 3, 12, 0, 0)); // 3 января 2026
